...
async fn handle_index_plugin_job(job: IndexPluginJob, q: Data<Arc<JobQueue>>) -> Result<(), BoxDynError> {
...
async fn handle_scrape_plugin_job(job: ScrapePluginJob, q: Data<Arc<JobQueue>>) -> Result<(), BoxDynError> {
    crate::flows::scrape_item::run_plugin(&job, &q).await;
    Ok(())
}

async fn handle_parse_scrape_results_job(job: ParseScrapeResultsJob, q: Data<Arc<JobQueue>>) -> Result<(), BoxDynError> {
...
async fn handle_content_service_job(_job: ContentServiceJob, q: Data<Arc<JobQueue>>) -> Result<(), BoxDynError> {
...
pub fn start_workers(queue: Arc<JobQueue>) -> Monitor {
    ...
    let m = Monitor::new();
    let m = register_worker!(m, queue, "riven-index",         index_storage,         n, handle_index_job);
    let m = register_worker!(m, queue, "riven-index-plugin",  index_plugin_storage,  n, handle_index_plugin_job);
    let m = register_worker!(m, queue, "riven-scrape",        scrape_storage,        n, handle_scrape_job);
    let m = register_worker!(m, queue, "riven-scrape-plugin", scrape_plugin_storage, n, handle_scrape_plugin_job);
    let m = register_worker!(m, queue, "riven-parse",         parse_storage,         n, handle_parse_scrape_results_job);
    let m = register_worker!(m, queue, "riven-download",      download_storage,      n, handle_download_job);
    let m = register_worker!(m, queue, "riven-content",       content_storage,       1, handle_content_service_job);
    m
}
