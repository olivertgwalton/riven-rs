//! Password hashing that understands both the legacy scrypt hashes and Argon2.
//!
//! The TypeScript better-auth frontend hashed with **scrypt**; everything
//! since hashes with **Argon2**. Without this, every user migrated from that
//! era is locked out, because their stored hash would never verify.
//!
//! So [`DualFormatHasher::verify`] accepts either format, and
//! [`DualFormatHasher::hash`] always writes Argon2 — new and changed passwords
//! are Argon2, old ones keep working. The first sign-in that verifies a scrypt
//! hash rewrites it as Argon2 in the background, keyed on the hash's own
//! value, so the legacy format drains away on its own.
//!
//! # scrypt parameters
//!
...
impl DualFormatHasher {
    ...
    fn verify_scrypt(salt_hex: &str, key_hex: &str, password: &str) -> anyhow::Result<bool> {
        ...
        let expected =
            hex::decode(key_hex).map_err(|error| anyhow!("malformed scrypt hash: {error}"))?;
        ...
        .map_err(|error| anyhow!("scrypt failed: {error}"))?;
    ...
    fn verify_argon2(hash: &str, password: &str) -> anyhow::Result<bool> {
        let parsed =
            PasswordHash::new(hash).map_err(|error| anyhow!("malformed argon2 hash: {error}"))?;
        Ok(Argon2::default()
            .verify_password(password.as_bytes(), &parsed)
            .is_ok())
    }

    fn hash_argon2(password: &str) -> anyhow::Result<String> {
...
async fn off_executor<T, F>(work: F) -> anyhow::Result<T>
where
...
impl DualFormatHasher {
    ...
    pub async fn verify(&self, hash: &str, password: &str) -> anyhow::Result<bool> {
