...
/** Shape raw TMDB list payloads (recommendations / similar) into list items. */
...
export function parseTMDBMovieDetails(data: TMDBMovieDetailsExtended | null) {
    ...
    return {
        ...
        collection: data.belongs_to_collection
            ...
            : null,
        trakt_recommendations: [] as TMDBTransformedListItem[]
    };
}
...
export function parseTVDBShowDetails(data: TVDBBaseItem | null) {
    if (!data) return null;
    ...
    const trakt_recommendations: TMDBTransformedListItem[] = [];
