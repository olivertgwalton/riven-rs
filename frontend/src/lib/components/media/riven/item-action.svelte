<script lang="ts">
    ...
    /** Each action is gated on its own capability, matching the resolver that
     * serves it — a manager-minus role may hold `retry` without `delete`. */
    const CAPABILITY: Record<ItemActionKind, Capability> = {
        reset: "RESET_ITEMS",
        retry: "RETRY_ITEMS",
        delete: "DELETE_ITEMS",
        pause: "PAUSE_ITEMS"
...
{#if can(page.data.permissions, CAPABILITY[kind])}
