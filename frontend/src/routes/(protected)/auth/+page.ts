...
import { authClient, type AuthUser, type LinkedAccount } from "$lib/auth-client";
...
export const load: PageLoad = async ({ parent }) => {
    ...
    const [passwordChangeForm, emailChangeForm, changeUserDataForm, createUserForm] =
        await Promise.all([
    ...
    const [accounts, managedUsers] = await Promise.all([accountsTask, managedUsersTask]);
