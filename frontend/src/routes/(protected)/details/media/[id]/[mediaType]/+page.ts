    ...
    try {
        ...
        if (mediaType === "movie") {
            const [tmdbResult, traktResult, rivenResult] = await Promise.all([
                ...
                }>(TRAKT_RECOMMENDATIONS_QUERY, { id, idType: "tmdb", mediaType: "movie" })
                    .then((data) => mapTraktRecommendations(data.traktRecommendations))
                    .catch((err) => {
                        logger.error(`Trakt fetch failed for movie id=${id}:`, err);
                        return null;
                    }),
                gqlClient<{ mediaItemStateByTmdb: GqlMediaItemStateTree | null }>(
                    MEDIA_ITEM_STATE_BY_TMDB_QUERY,
                    { tmdbId: id }
                )
            ...
            const parsedDetails = parseTMDBMovieDetails(details as TMDBMovieDetailsExtended);
            if (!parsedDetails) {
            ...
            const [tvdbEpisodesResult, tvdbTranslationsResult, traktResult, rivenResult] =
                await Promise.all([
                    normalizeFetch(
                        fetchTvdbSeriesExtended<{ data: TVDBBaseItem }>(tvdbId, "episodes").then(
                    ...
                    normalizeFetch(
                        ...
                        }>(tvdbId, "translations").then((data) => ({ data, error: null }))
                    ),
                    gqlClient<{
                        traktRecommendations: GqlTraktRecommendation[];
                    ...
                    gqlClient<{ mediaItemStateByTvdb: GqlMediaItemStateTree | null }>(
                        MEDIA_ITEM_STATE_BY_TVDB_QUERY,
            ...
            const parsedDetails = parseTVDBShowDetails(validatedData);
            if (!parsedDetails) {
