...
export const load: PageLoad = async ({ parent }) => {
    ...
    try {
        const [
            ...
            gqlClient<{ settingsSections: SettingsSection[] }>(SETTINGS_SECTIONS_QUERY, {}).catch(
                () => ({ settingsSections: [] })
            ),
            gqlClient<{ qualityProfiles: QualityProfile[]; customProfiles: CustomProfile[] }>(
                RANK_PROFILES_QUERY,
                {}
            ...
            gqlClient<{ instanceStatus: InstanceStatus }>(INSTANCE_STATUS_QUERY, {}).catch(() => ({
                instanceStatus: {
